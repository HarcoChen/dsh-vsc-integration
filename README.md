...
**How is Jev integrated?** Internal builds carry the IDE-neutral `dsh-jev-integration` Runtime package and mount it directly into a Runtime started by this extension. No second IDE plugin is required, and an already-running or externally managed Runtime is never modified. The integration is disabled by default; enabling `dsh.jev.enabled` sends complete arguments for the configured guarded tools to the TypeSafe System One endpoint. The nested `dsh.jev.loopGuard.*`, `dsh.jev.resultShaper.*`, and `dsh.jev.doneGate.*` settings are passed unchanged to the mounted plugin; the first two are the optional loop-avoidance and repetitive-output shaping policies, while the evidence gate is a quality guard. Use **DSH: Configure Jev API Key** to save the key in VS Code SecretStorage; the extension passes it only to Runtime processes it starts. `TYPESAFE_API_KEY` and the existing `$HOME/.dsh/.env` entry remain supported as fallbacks. This build exposes the package's minimal advisory hook and Runtime policies, not the full upstream `dsh-jev` bundle.
