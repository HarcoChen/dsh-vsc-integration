...
- **需协议**：Harness 当前公开 Web RPC 不足，需要先扩展或稳定协议。
...
| 当前文件/选区 | 手动附加 | 自动感知、行号引用 | 自动共享 | `session.prompt` |
...
| Fork/checkpoint | 无 | fork、代码 rewind | Server 有 fork/revert | `session.fork`；代码 rewind 需自建/新协议 |
| 模型/推理强度 | 无 | 可切换 | TUI 可切换 | `session.models/selectModel` |
| 命令/Skills | 无 | `/` 菜单和 skills | TUI 命令 | `skill.list`；无 command catalog RPC |
| MCP/插件 | 无 | MCP 与插件管理 UI | Server 有 MCP API | 需补 Harness Web API |
...
- [x] 手动附加当前文件、选区、文件、文件夹路径、当前文件 diagnostics 和 unstaged Git diff。
- [x] 发送前预览、复制、移除和清空 IDE context。
...
- [ ] **Harness 能力握手**（Harness 直连）：调用 `host.describe`，记录 runtime 版本、方法/模块和兼容性；不支持的 UI 隐藏并解释原因。
...
- [ ] **权限 preset**（部分需协议）：读取 `permissions` projection；推动稳定 mutation RPC，未提供前明确回退到 Harness 命令。
...
- [ ] **Checkpoint**（需协议/IDE 自建）：建立 turn 到文件快照映射，提供仅 fork、仅恢复代码、fork 并恢复代码。
...
- [ ] **自动选区感知**（IDE 自建）：footer 显示当前文件/选区行数，发送的实际内容始终可见。
...
- [ ] **Plan mode 开关**（Harness 直连）：读取 `plan` projection，显示 active/pending，通过稳定命令或 RPC 切换。
...
- [ ] **Background jobs**（Harness 直连）：展示 `session/jobs` 快照、进度、结果和停止状态。

### P1-2 Commands、Skills、MCP 与插件

- [ ] **`/` 命令菜单**（部分需协议）：搜索、参数提示和执行；Harness 应补公开 command catalog RPC，客户端不硬编码。
- [ ] **Skills 浏览器**（Harness 直连）：使用 `skill.list` 展示名称、说明、可调用性和来源；插入调用而非静默注入全文。
- [ ] **MCP 状态**（需协议）：展示 server、连接状态、工具数、失败、重连和 OAuth；credential 仍由 Harness 管理。
- [ ] **Harness 插件管理**（需协议）：查看安装/启用状态、版本、配置 schema 和加载错误；安装升级需来源与重启提示。
...
- [ ] **Compact**（Harness 直连）：支持手动触发、运行状态、压缩范围和失败原因。
- [ ] **使用量归因**（Harness 直连/需协议）：按模型、skill、subagent、tool/plugin 展示 token、耗时和可用成本，不猜缺失价格。
...
建议推动 Harness 新增或稳定的协议：

- [ ] `command.list/execute` 的目录和参数 schema，避免客户端硬编码 slash command。
- [ ] permission preset 与 plan mode 的直接 mutation RPC，避免借普通 prompt 模拟控件。
- [ ] MCP 与 Harness plugin 的状态、配置、安装、升级和卸载 API。
- [ ] turn/message 对应的文件变更、proposal diff 和 checkpoint/revert API；若 runtime 不拥有该语义，应明确由 IDE 实现。
- [ ] 版本化 capability declaration 和兼容窗口；Harness 处于 developer preview，不能只靠调用失败探测。
- [ ] 远程 runtime 的认证、TLS 和 origin 约定；当前 Web Server 文档明确默认无这些策略。
...
Session 列表、恢复、多 tab、queue/steer
          ↓
文件快照、原生 diff、checkpoint
          ↓
@ 引用、Plan、模型、Goal、Subagent、Token
          ↓
Skills / MCP / 插件 / 远程 / Worktree
```
