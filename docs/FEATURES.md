...
本文以 Claude Code for VS Code、OpenCode VS Code 扩展和 DeepSeek Harness Web/API 为基准，列出 DSH IDE 需要实现或接入的功能。Harness runtime/client 已具备的能力不应在本仓库重复实现；本扩展只负责 adapter、Webview bridge 和 VS Code 特有体验。

标记说明：

- **[x] 已实现**：本仓库已经提供并验证对应的 VS Code 体验。
- **[adapter] Harness 接入**：Harness runtime/client 已提供协议、投影或 browser-safe UI，本仓库只需接入、包装和复用。
- **[ide] IDE 自建**：必须使用 VS Code API 或 DSH IDE 自己的安全/交互能力完成。
- **[adapter/ide] 混合**：协议/UI 复用与 VS Code 安全边界都需要工作。
- **[out] 非目标**：竞品具备但 Harness 当前公开 Web RPC 未开放；仅记录差距，不通过猜测 endpoint 或普通 prompt 实现。
...
DeepSeek Harness 已提供完整 GUI 所需的大部分后端和 client/UI 基础：事件溯源 session、WebSocket mux、审批、用户问题、plan、permission preset、模型、队列、附件、workspace、skill、goal、subagent、job、token/context projection 和 compaction。DSH IDE 当前的主要差距是 adapter 尚未消费这些能力，而不是缺少聊天输入框。
...
| 子 agent/job/todo | [x] 基础实现；UI 待复用 | tool activity、todo、subagent | Server 有 todo/agent | `subagent.*`、jobs、goal projection |
| 终端输出上下文 | 无 | `@terminal:name` | 原生终端形态 | IDE 自建 |
| 图片附件 | [adapter] | 有 | message parts 支持 | `session.attachment` + `imageLimits` |
...
- [x] **WebSocket mux 客户端**（Harness 直连）：消费 session mux 和 host stream；处理 event、subscribed、queue、jobs、projection、host status/error 等 frame。
...
- [adapter] **权限状态**（Harness 直连）：复用 `permissions` projection 和 Harness permission UI；当前 RPC 没有直接 mutation 方法，因此不提供切换控件。
- [x] **fail-closed**（IDE 自建）：断线、过期、重复应答或解析失败都不视为授权，显示 resolved/cancelled/unavailable。
- [adapter] **权限/风险说明**（Harness 直连）：复用 Harness permission preset、projection 和 permission UI；本仓库只负责 Webview/VS Code 容器适配，不自行根据 tool 名称猜测风险等级。
...
- [adapter] **搜索与历史分页**（Harness 直连）：复用 `session.search` 和 `session.history`；本仓库仍需接入跨 session 结果、上翻加载和滚动锚点。
- [x] **新建、切换、重命名、归档**（Harness 直连）：复用同工作区 blank session，由 host stream 收敛 UI。
- [ide] **多会话并行**（IDE 自建）：每个 session 可作为 editor tab 打开；状态、草稿、context 和事件彼此隔离。
- [ide] **关闭与恢复**（IDE 自建）：恢复最近关闭的 tab；持久化草稿但不持久化敏感附件内容。
- [x] **Fork 对话**（Harness 直连）：从已完成 turn 的 seq 调用 `session.fork`，显示 parent/child。

### P0-5 文件变更审查与恢复

- [ide] **任务前快照**（IDE 自建）：记录 Git HEAD、tracked/untracked 状态和目标文件摘要；多根工作区分别记录。
- [ide] **变更集合**（IDE 自建）：按 turn 展示新增、修改、删除和 rename，不把任务前用户改动归给 agent。
- [ide] **VS Code 原生 diff**（IDE 自建）：逐文件打开左右对比，支持路径/行号跳转。
- [ide] **接受/拒绝/反馈**（IDE 自建 + Harness 审批）：未执行写操作走审批；已落盘变化可逐文件保留、恢复或继续修改。
- [out] **Checkpoint/code rewind**：当前 Harness RPC 仅支持 `session.fork`，本扩展不实现 runtime checkpoint/revert。
- [ide] **冲突保护**（IDE 自建）：恢复前检测用户或其他任务的后续修改；冲突时只展示 diff，不覆盖。
...
- [ide] **敏感信息拦截**（IDE 自建）：识别密钥、私钥、`.env` 和超大内容，阻止或要求明确确认。
...
- [ide] **Webview 安全**：严格 CSP、nonce、消息 schema、Markdown sanitize、command URI 白名单。
- [ ] **诊断包**：默认只导出脱敏版本、配置、状态和协议错误，不包含 prompt、文件、credential 或图片。
- [ ] **性能预算**：长 session 虚拟列表；增量合并不全量重渲染；大型 diff/附件有硬上限。

## P1：成熟开发体验

完整的 Trace 交互方案见 [DSH Session Trace 的 VS Code 集成设计](TRACE_INTEGRATION.md)。Trace MVP 与流式聊天共享事件基础设施，建议在 P0 WebSocket 状态仓库后立即交付。

### P1-1 Plan、模型和 agent 控制

- [adapter] **Plan 状态**（Harness 直连）：复用 `plan` projection 和 Harness plan UI；当前 RPC 没有直接切换方法，因此不提供模式开关。
- [ide] **计划文档批注**（IDE 自建）：把 plan 打开为 Markdown 文档，收集 inline comment 后作为一次反馈返回。
- [adapter] **模型与 reasoning effort**（Harness 直连）：接入 `session.models/selectModel` 和 Harness model UI，展示 routable、provider 分组、失败原因和当前选择。
- [adapter] **Agent preset**（Harness 直连）：接入 preset 列出、读取、选择；新建 session 时明确使用哪个 preset。
...
### P1-2 Skills、配置与凭据
- [adapter] **Skills 浏览器**（Harness 直连）：复用 `skill.list` 和 Harness skill UI，展示名称、说明、可调用性和来源；插入调用而非静默注入全文。
- [adapter] **配置编辑器**（Harness 直连）：复用 `settings.describe/update` 和 schema 表单，保留打开原始配置文件入口。
- [adapter/ide] **Credential UI**（Harness 直连）：接入 `credentials.describe/set/unset`；值不回显、不进入 webview state 或日志，并由 Extension Host 保管秘密。

MCP 和 Harness 插件管理属于对标差距，当前 Web RPC map 未公开对应方法，不进入实现范围。

### P1-3 Context、附件和用量

- [adapter] **图片附件**（Harness 直连）：复用 `imageLimits` 和 Harness attachment UI，客户端先校验数量、类型和大小，再走 `session.attachment`。
- [ide] **Problems/测试上下文**（IDE 自建）：附加失败测试、Problems、Task 输出，保留来源与截断提示。
- [ide] **终端输出引用**（IDE 自建）：按 terminal 和明确范围附加，默认脱敏环境变量与控制字符。
- [adapter] **Token/context 指示器**（Harness 直连）：消费 `tokenUsage`、`contextPressure`、`contextBreakdown`，标明估算或实测。
- [adapter] **Compact 可观察性**（Harness 直连）：复用事件和 projection 中的运行状态、压缩范围和失败原因；不提供手动触发按钮。
- [adapter] **使用量归因**（Harness 直连）：只展示现有 projection 明确提供的 token、context 和 cache 字段，不推算缺失成本。
...
- [adapter/ide] **Workspace 管理**（Harness 直连）：复用 `workspace.list/create/rename/delete/order/archiveSession`；由 IDE 负责区分 VS Code workspace 与 Harness workspace、cwd 和 URI 映射。
...
- session/host WebSocket streams、HTTP `/api/respond`
