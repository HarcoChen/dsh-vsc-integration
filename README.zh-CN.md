...
多个 VS Code 窗口优先复用共享锁记录的 Runtime，再探测端口 `3080`。已知版本过旧时要求升级；没有版本信息或没有本扩展锁文件的服务，允许在认证及 `session/list` 调用成功后直接复用。外部服务不会归本扩展所有，断开连接不会停止它。需要认证但缺少凭据时，会提示将 `dsh.serverUrl` 设为完整启动 URL（包含 token）。

没有可复用服务时，默认优先使用空闲的 `3080`；非 DSH 占用或探测结果不明确时使用操作系统分配的回环端口。启动前再次发现可用 DSH 则直接复用。扩展通过系统临时目录中的 `dsh-runtime.lock` 发布自有 Runtime 的地址、凭据、版本和进程信息，供其他窗口复用。

锁清理规则：

- 残留锁自动恢复：损坏或半写入的普通锁文件保留 2 秒写入宽限期，随后在互斥保护下重新核对文件身份和内容并删除；不再要求用户手动删锁。
- 所有者已退出且未记录地址的空锁自动回收；记录了启动器或进程组时，要求这些进程也已退出。已公布地址的锁仍需确认监听端口已关闭。没有地址的旧包装启动器在所有已记录进程退出后也允许恢复，之后通过正常服务发现尽量复用仍在监听的服务。
- 已记录的 Runtime 已退出、监听端口已关闭时，即使原编辑器仍然存活，也自动回收锁。只有编辑器 PID、尚无 Runtime 身份的有效启动锁继续参与互斥，避免多个窗口同时启动。
- 锁文件清理不会终止进程。仍在监听的服务优先复用；已知版本过旧或不响应、且能核实 DSH 入口身份的孤儿进程，仍需确认后才能停止并重启。坏锁或未公布地址的旧锁可能无法提供完整进程信息，因此自动回收不代表已证明所有未记录的子进程都退出。
- 正常退出先停止本扩展拥有的进程树，再释放锁。POSIX 使用独立进程组，Windows 在根进程身份仍可确认时使用限定 PID 的 `taskkill /T`。释放同时核对所有者、文件身份和内容，不删除其他窗口替换后的锁。
- 创建、发布和删除通过内核互斥及 `dsh-runtime.lock.mutation` 文件串行化。所有者已退出的 mutation 文件自动回收；损坏的 mutation 文件同样经过 2 秒宽限期后自动恢复。存活的有效 mutation 所有者仍保持互斥。
