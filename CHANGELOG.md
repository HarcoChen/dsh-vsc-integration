...
- 移除 Runtime 启动锁，改用只读的 Runtime 广告做多窗口发现。广告不再授予或否决启动权限：缺失、过期或无法读取的广告都不会阻塞启动，最坏只是多一次失败的健康探测。启动前通过短暂的回环 mutex 协调并复查共享 Runtime，抢不到也照常启动自己的实例。
- 每个窗口只发布并撤回自己的 `<ownerId>.json`，不再有需要回收的共享写所有权；旧的 `dsh-runtime.lock` 仅作只读提示。恢复流程的会话租约改用独立的 `fileLease` 模块，保留原有的内核互斥与文件身份校验。

### 修复

- Windows 上 pnpm/npx 包装进程退出而 Runtime 仍在运行时，不再出现「另一个 dsh Runtime 正在启动」并空等 120 秒：启动许可已不存在，因此不会再被自己上一次失败的启动挡住。
- 启动器意外退出后，只要端点仍然应答就保留其广告，让恢复流程直接接管幸存的 Runtime，而不是另起一个；只有明确被拒绝的回环连接才撤回广告。
- 识别包管理器存储目录结构损坏（能解析出 DSH 但解析不到它的依赖，如 `ERR_MODULE_NOT_FOUND`）。这类失败重试 registry 没有意义，改为直接换用 npx 重试，并在最终错误中给出需要清理的存储路径。
...
- Windows 下 pnpm/npx 在发布 Runtime 地址前报告下载错误时，按失败启动器清理残留锁，避免下一次启动误报已有 Runtime 正在启动。
- 正确的清理归档对话，不再出现在候选
