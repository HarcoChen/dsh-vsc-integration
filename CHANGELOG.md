...
- 支持 npx 的 `--package`/`-p` 版本探测并区分包参数与 Runtime 端口参数；修复写锁 staging 清理失败时的句柄与 guard 残留，以及强制停止前进程恰好退出导致恢复中断的问题。
- 自动回收所有者已退出的 `.mutation` 写锁；使用随进程退出自动释放的本机互斥入口串行化回收，并原子发布完整锁记录，避免崩溃留下空锁。保留旧版文件锁互斥，存活或无法确认所有者的锁不会被删除。
- 升级遗留的 DSH 孤儿进程可确认停止、清理共享锁并重新启动；兼容版本但 RPC 无响应的孤儿也提供该入口。进程未在等待期退出时，重新核对锁和进程身份后强制终止；仍存活的编辑器、无关进程及发生替换的锁不会被清理。
- 修复 pnpm/npx 显式使用 `@deepseek-ai/dsh@next`、`@latest` 等版本选择器时误报 Runtime 版本未知：启动前探测实际版本，通过最低版本检查后固定到该版本启动，共享锁记录实际版本。
