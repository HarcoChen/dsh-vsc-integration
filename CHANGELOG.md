...
- 默认 Runtime 保留 `0.1.5-rc.1`；本机及共享锁按 SemVer 接受所有不低于最低版本 `0.1.5-rc.1` 的版本，记录实际版本，不维护逐版本兼容列表。审计覆盖上游 master `c291e7961a` 与 RC.2 发布契约 `fb2c4b9e698e30edb738bca4cf0618587db7d203`。
- 本机 CLI 不兼容时先询问升级，用户同意后才升级可确认的旧版 npm 全局安装并重新探测；拒绝后再走 pnpm/npx/CNB。升级失败可选择回退或取消，诊断命令不弹窗或安装，启动取消后不会因迟到的确认继续升级。
- 兼容 master 的可选模式选择策略：关闭后隐藏 IDE 的 `/mode`、`/preset` 候选，清除草稿模式，并在首次发送前将空会话恢复为有效默认模式；设置更新和重连会刷新策略。旧 Runtime 未返回该字段时仍允许选择。
- 保留消息反馈的分类，避免切换评价、编辑说明或处理版本冲突时丢失其他客户端写入的数据；Skill 补全悬浮提示支持上游新增的文件路径。
- CNB 的 `0.1.5-rc.2` 独立 Runtime 镜像仍返回 404；官方 npm Runtime 已通过本地模型桩的隔离集成验证。
