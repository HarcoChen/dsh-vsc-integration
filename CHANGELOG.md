...
- 内测构建内置最小 `dsh-jev-integration` Runtime 集成，默认关闭，可通过 `dsh.jev.*` 配置启用。
...
- `DSH: Open Chat in Editor Tab` 把当前会话镜像到编辑器标签页
- 审批放行前会检查目标文件在编辑器里是否有未保存改动
- 支持没啥用的反馈功能
...
- 每个窗口只发布并撤回自己的 `<ownerId>.json`，不再有需要回收的共享写所有权
